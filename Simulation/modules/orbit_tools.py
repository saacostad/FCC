...
def avermax(twiss_data, Xs, Ys, arc, ref_bpm, treshold = np.pi/2, log = False):
    ...
    for col in Xs_filtered.columns:
        ...
        if abs(neg_Delta_delta_x) < treshold:
    ...
    avermax_x = np.mean(np.array(x_turns), axis = 0) * 1000.
    avermax_y = np.mean(np.array(y_turns), axis = 0) * 1000.
